...
// ... (All other init...Page functions are unchanged from the final, feature-complete versions) ...
...
function renderSubmissions(submissions) {
    ...
    if (submissions.length === 0) {
        ...
        const rowsHTML = submissions.map(s => `<tr><td>${s.student_name}</td><td>${s.task_title}</td><td class="submission-actions"><button class="btn btn-green" onclick="approveSubmission('${s.id}')">Approve</button><button class="btn btn-red" onclick="rejectSubmission('${s.id}')">Reject</button></td></tr>`).join('');
        content = `<h2>Pending Submissions</h2><table class="list-table"><thead><tr><th>Student</th><th>Task</th><th>Actions</th></tr></thead><tbody>${rowsHTML}</tbody></table>`;
    }
...
function renderRoster(roster) {
    ...
    if (roster.length === 0) {
        ...
        const rowsHTML = roster.map(s => `<tr><td>${s.full_name}</td><td>${s.class_name}</td><td>${s.points}</td></tr>`).join('');
        content = `<h2>Student Roster</h2><table class="list-table"><thead><tr><th>Name</th><th>Class</th><th>Points</th></tr></thead><tbody>${rowsHTML}</tbody></table>`;
    }
    container.innerHTML = content;
}
